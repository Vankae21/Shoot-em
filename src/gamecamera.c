...
GameCamera* init_camera(Player* player)
{
    ...
    gcamera->camera->offset = (Vector2){ WIDTH/2, HEIGHT/2 };
    gcamera->camera->zoom = (float)WIDTH/(float)DEF_WIDTH;
...
void update_camera(GameCamera* gcamera, Vector2 max_frame)
{
    ...
    if(mod_target.x - WIDTH/(2*gamecamera->camera->zoom) < 0)
	{
		mod_target.x = WIDTH/(2*gamecamera->camera->zoom);
	}
	else if(mod_target.x + WIDTH/(2*gamecamera->camera->zoom) > max_frame.x)
	{
		mod_target.x = max_frame.x - WIDTH/(2*gamecamera->camera->zoom);
	}
	if(mod_target.y - HEIGHT/(2*gamecamera->camera->zoom) < 0)
	{
		mod_target.y = HEIGHT/(2*gamecamera->camera->zoom);
	}
	else if(mod_target.y + HEIGHT/(2*gamecamera->camera->zoom) > max_frame.y)
	{
		mod_target.y = max_frame.y - HEIGHT/(2*gamecamera->camera->zoom);
	}
